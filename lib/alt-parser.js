...
ZeroOrMore.prototype.parse = function (grammar, params, view) {
    ...
    var next = result;
    var iterations = 0;
    var maxIterations = 10000; // 防止无限循环的最大迭代次数
    var lastPosition = view.getEnd();
    ...
    do {
        ...
        // 检查全局超时状态
        if (typeof global !== 'undefined' && global.jiraParserTimeout) {
            throw new Error('ZeroOrMore: Operation timed out after 5 seconds');
        }

        // 检查是否超过最大迭代次数
        if (++iterations > maxIterations) {
            throw new Error('ZeroOrMore: Maximum iterations exceeded, possible infinite loop detected');
        }

        next = result.parseAndMerge(grammar, params, this.zom_pred);

        // 检查输入位置是否推进，防止零长度匹配导致的死循环
        var currentPosition = next.pt_view ? next.pt_view.getEnd() : lastPosition;
        if (!next.failed() && currentPosition === lastPosition) {
            throw new Error('ZeroOrMore: Input position not advancing, possible infinite loop detected');
        }
...
OneOrMore.prototype.parse = function (grammar, params, view) {
    ...
    var iterations = 1; // OneOrMore至少需要一次匹配
    var maxIterations = 10000; // 防止无限循环的最大迭代次数
    var lastPosition = view.getEnd();
    ...
    do {
        ...
        // 检查全局超时状态
        if (typeof global !== 'undefined' && global.jiraParserTimeout) {
            throw new Error('OneOrMore: Operation timed out after 5 seconds');
        }

        // 检查是否超过最大迭代次数
        if (++iterations > maxIterations) {
            throw new Error('OneOrMore: Maximum iterations exceeded, possible infinite loop detected');
        }
        ...
        var currentPosition = next.pt_view ? next.pt_view.getEnd() : lastPosition;
        if (!next.failed() && currentPosition === lastPosition) {
            throw new Error('OneOrMore: Input position not advancing, possible infinite loop detected');
        }
        lastPosition = currentPosition;
    } while (!next.failed());
...
Grammar.prototype._buildRegExCaches = function () {
    ...
    var regexIterations = 0;
    var maxRegexIterations = 100; // 防止正则表达式生成无限循环
    ...
    do {
        ...
        // 检查全局超时状态
        if (typeof global !== 'undefined' && global.jiraParserTimeout) {
            throw new Error('Grammar.resolveRegExStrs: Operation timed out after 5 seconds');
        }

        // 检查是否超过最大迭代次数
        if (++regexIterations > maxRegexIterations) {
            throw new Error('Grammar.resolveRegExStrs: Maximum iterations exceeded, possible circular dependency in grammar rules');
        }
